...
if (!next) {
  console.error(
    `version check: "${stagedVersion}" is not a valid MAJOR.MINOR.PATCH version`,
  );
  process.exit(1);
}
...
if (!isProperBump) {
  console.error(
    `version check: staged version is ${stagedVersion}, but HEAD is ${headVersion}`,
  );
  console.error(
  ...
  if (stagedVersion === headVersion) {
    console.error(
      "bump the version in package.json and stage it with: git add package.json",
    );
  }
